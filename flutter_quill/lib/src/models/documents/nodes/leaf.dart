...
abstract class Leaf extends Node {
  ...
  void adjust() {
    ...
    var node = this as TextLeaf;

    // Merging it with previous node if style is the same.
    final prev = node.previous;
    if (!node.isFirst && prev is TextLeaf && prev.style == node.style) {
    ...
    if (!node.isLast && next is TextLeaf && next.style == node.style) {
...
class TextLeaf extends Leaf {
  ...
  @override
  Node newInstance() => TextLeaf(value);
